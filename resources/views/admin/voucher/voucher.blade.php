...
<div x-data="voucherHandler()" x-init="fetchVouchers()">
    <div class="px-5 mb-4 flex items-center justify-between">
        ...
        <div class='w-full grid grid-cols-2 lg:flex lg:justify-end gap-x-4 md:gap-x-2'>
            <button @click="createVoucherModal = true" class="flex items-center justify-center px-6 py-2 bg-red-600 text-white font-medium text-md  rounded-lg hover:bg-red-700 focus:bg-red-700 focus:outline-none focus:ring-0 active:bg-red-800 ">
    ...
    <!-- Edit Voucher Modal -->
    <div x-show="showEditModal" class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
        <div class="relative w-full h-full max-w-2xl md:h-auto mx-auto mt-10 shadow-lg ">
            <div x-cloak @click="showEditModal = false" x-show="showEditModal" 
                class="fixed inset-0 transition-opacity bg-gray-500 opacity-40" aria-hidden="true"
            ></div>
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="p-6 space-y-6">
                    <form @submit.prevent="submitEdit" id="editForm" method="POST">
                        @csrf
                        <div class="grid grid-cols-2 gap-4">
                            <div class="mb-4 col-span-2">
                                <label for="code" class="block text-sm font-medium text-gray-700">Code</label>
                                <input type="text" name="code" x-model="selectedVoucher.code" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm" maxlength="6" required>
                            </div>
                            <div class="mb-4">
                                <label for="expire_date" class="block text-sm font-medium text-gray-700">Expire Date</label>
                                <input type="date" name="expire_date" x-model="selectedVoucher.expired_date" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm" required>
                            </div>
                            <div class="mb-4">
                                <label for="quota" class="block text-sm font-medium text-gray-700">Quota</label>
                                <input type="number" name="quota" x-model="selectedVoucher.quota" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm" required>
                            </div>
                            <div class="mb-4">
                                <label for="discount_price" class="block text-sm font-medium text-gray-700">Discount Price</label>
                                <input type="text" name="discount_price" x-model="selectedVoucher.discount_price" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm">
                            </div>
                            <div class="mb-4">
                                <label for="discount_percentage" class="block text-sm font-medium text-gray-700">Discount Percentage</label>
                                <input type="text" name="discount_percentage" x-model="selectedVoucher.discount_percentage" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm">
                            </div>
                            <div class="mb-4">
                                <label for="max_discount" class="block text-sm font-medium text-gray-700">Maximal Discount</label>
                                <input type="text" name="max_discount" x-model="selectedVoucher.max_discount" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm">
                            </div>
                            <div class="mb-4">
                                <label for="min_price" class="block text-sm font-medium text-gray-700">Minimal Price</label>
                                <input type="text" name="min_price" x-model="selectedVoucher.min_price" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm">
                            </div>
                        </div>
                ...
                <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                    <button type="submit" @click="submitEdit" class="text-white inline-flex items-center bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
    ...
    <div x-show="showDeleteModal" class=" overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative w-full h-full max-w-md md:h-auto mx-auto mt-10 shadow-lg ">
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <button @click="submitDelete" type="submit" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                        Delete
                    </button>
    ...
    <!-- Modal -->
    <div x-show="createVoucherModal" x-transition @keydown.escape.window="createVoucherModal = false" class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
        <div class="relative w-full h-full max-w-2xl md:h-auto mx-auto mt-10 shadow-lg ">
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <div class="flex items-start justify-between p-4 border-b rounded-t">
                    ...
                    <button @click="createVoucherModal = false" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                        </svg>
                    </button>
                ...
                <!-- Modal body -->
                <div class="p-6 space-y-6">
                    <form @submit.prevent="tambahVoucher" id="voucherForm" method="POST">
                        @csrf
                        <div class="grid grid-cols-2 gap-4">
                            <div class="mb-4 col-span-2">
                                <label for="code" class="block text-sm font-medium text-gray-700">Code</label>
                                <input type="text" name="code" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm" maxlength="6" required>
                            </div>
                            <div class="mb-4">
                                <label for="expire_date" class="block text-sm font-medium text-gray-700">Expire Date</label>
                                <input type="date" name="expire_date" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm" required>
                            </div>
                            <div class="mb-4">
                                <label for="quota" class="block text-sm font-medium text-gray-700">Quota</label>
                                <input type="number" name="quota" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm" required>
                            </div>
                            <div class="mb-4">
                                <label for="discount_price" class="block text-sm font-medium text-gray-700">Discount Price</label>
                                <input type="text" name="discount_price" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm">
                                <input type="hidden" name="discount_price_hidden" id="discount_price_hidden">
                            </div>
                            <div class="mb-4">
                                <label for="discount_percentage" class="block text-sm font-medium text-gray-700">Discount Percentage</label>
                                <input type="text" name="discount_percentage" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm">
                                <input type="hidden" name="discount_percentage_hidden" id="discount_percentage_hidden">
                            </div>
                            <div class="mb-4">
                                <label for="max_discount" class="block text-sm font-medium text-gray-700">Maximal Discount</label>
                                <input type="text" name="max_discount" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm">
                                <input type="hidden" name="max_discount_hidden" id="max_discount_hidden">
                            </div>
                            <div class="mb-4">
                                <label for="min_price" class="block text-sm font-medium text-gray-700">Minimal Price</label>
                                <input type="text" name="min_price" class="block w-full mt-1 py-2 px-3 border-gray-300 rounded-md shadow-sm">
                ...
                <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b">
                    <button type="submit" @click="tambahVoucher" class="text-white inline-flex items-center bg-red-700 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                        <svg class="me-1 -ms-1 w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd"></path></svg>
                        Tambah Voucher
                    </button>
                    <button @click="createVoucherModal = false" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-2 focus:outline-none focus:ring-gray-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10">Cancel</button>
                </div>
            </div>
        </div>
    </div>
    ...
    return {
        ...
        async submitEdit() {
            const updateData = {
                ...
                expired_date: this.selectedVoucher.expired_date,
                quota: this.selectedVoucher.quota,
                discount_price: this.selectedVoucher.discount_price,
                discount_precentage: this.selectedVoucher.discount_precentage,
        ...
        async submitDelete() {
            try {
                const response = await axios.delete(`/vouchers/${this.selectedVoucher}`);
                this.showDeleteModal = false;
                this.fetchVouchers();
