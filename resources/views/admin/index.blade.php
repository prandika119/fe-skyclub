...
@section('content')
    {{-- Container utama untuk chart statistik --}}
    <div class="bg-white shadow rounded-lg border-gray-300 mb-4 p-6">
        <div class="flex justify-between items-start">
            <div>
                {{-- Label ini akan diupdate oleh JavaScript --}}
                <h5 id="total-bookings-label" class="leading-none text-xl font-bold text-gray-900 pb-2">...</h5>
                {{-- Total booking ini juga akan diupdate --}}
                <p class="text-base font-normal text-gray-500">Total bookings: <span id="total-bookings">-</span></p>
            </div>
            <div class="flex justify-end items-center">
                ...
                <div id="lastDaysdropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44">
                    <ul class="py-2 text-sm text-gray-700" aria-labelledby="dropdownDefaultButton">
                        {{-- Menggunakan data-atribute untuk menyimpan nilai --}}
                        <li><a href="#" class="block px-4 py-2 hover:bg-gray-100" data-days="1"
                                data-label="Today">Today</a></li>
                        <li><a href="#" class="block px-4 py-2 hover:bg-gray-100" data-days="2"
                                data-label="Yesterday">Yesterday</a></li>
                        <li><a href="#" class="block px-4 py-2 hover:bg-gray-100" data-days="7"
                                data-label="Last 7 days">Last 7 days</a></li>
                        <li><a href="#" class="block px-4 py-2 hover:bg-gray-100" data-days="30"
                                data-label="Last 30 days">Last 30 days</a></li>
                        <li><a href="#" class="block px-4 py-2 hover:bg-gray-100" data-days="90"
                                data-label="Last 90 days">Last 90 days</a></li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        {{-- Wadah untuk Chart --}}
        <div id="stats-chart" class="mt-4"></div>
    </div>
    ...
    {{-- <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
        <div x-data="bookingHandler()" x-init="fetchBookings()"
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('admin.booking.allBooking') }}" class="text-sm font-medium text-blue-600 hover:underline">
                    Lihat semua
                </a>
            </div>
            <div class="flow-root">
                            ...
                            <li class="py-3 sm:py-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0">
                                        <img class="w-8 h-8 rounded-full" :src="booking.user.profile_photo"
                                            :alt="booking.user.name">
                                    </div>
                                    <div class="flex-1 min-w-0 ms-4">
                                        <p x-text="booking.user.name" class="text-sm font-medium text-gray-900 truncate">
                                        </p>
        ...
        <div class="w-full p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8" x-data="rescheduleHandler()"
                    ...
                    <template x-for="reschedule in reschedules.slice(0, 5)" :key="reschedule.id">
                        <li class="py-3 sm:py-4">
                            <div class="flex items-center">
                                ...
                                <div class="flex-1 min-w-0 ms-4">
                                    <p class="text-sm font-medium text-gray-900 truncate" x-text="reschedule.user.name">
                                    </p>
                                    <div class="text-xs text-gray-500">
                                        <p class="font-semibold">Jadwal Lama:</p>
                                        <p x-text="reschedule.old_schedule.date">
                                        </p>
                                        <p class="font-semibold mt-1">Jadwal Baru:</p>
                                        <p x-text="reschedule.new_schedule.date">
                                        </p>
                                    </div>
                                </div>
                                <div class="inline-flex items-center text-base font-semibold text-gray-900">
                                    <button @click="setCurrentReschedule(reschedule.id)" data-modal-target="confirmModal"
                        ...
                        <li class="py-3 sm:py-4">
                            <div class="flex items-center">
                                ...
                                <div
                                    ...
                                    <button @click="setCurrentRequest(request.id)" data-modal-target="confirmModal"
                                        data-modal-toggle="confirmModal"
                                        class="block text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-xs px-3 py-1.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800"
                                        type="button">
                                        Action
...
@push('script')
        ...
        function bookingHandler() {
            return {
                ...
                async fetchBookings() {
                    this.isLoading = true;
                    try {
                        const response = await axios.get('/booking-stats');
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            // Objek untuk mengelola semua logika chart
            const statsChart = {
                chart: null, // Untuk menyimpan instance ApexCharts

                // Fungsi untuk mengambil data dari API
                async fetchData(days) {
                    // Tampilkan pesan loading sederhana
                    document.getElementById('stats-chart').innerHTML =
                        '<p class="text-center p-10">Loading chart data...</p>';
                    try {
                        // Ganti dengan path API Anda yang benar
                        const response = await axios.get(`booking-stats?days=${days}`);
                        // Pastikan mengakses data dari `response.data`
                        return response.data;
                    } catch (error) {
                        console.error("Gagal mengambil data untuk chart:", error);
                        // Tampilkan pesan error jika API gagal
                        document.getElementById('stats-chart').innerHTML =
                            '<p class="text-center p-10 text-red-500">Gagal memuat data chart.</p>';
                        return []; // Kembalikan array kosong agar tidak error
                    }
                },

                // Fungsi untuk merender atau mengupdate chart
                async renderChart(days = 7, label = 'Last 7 days') {
                    const apiData = await this.fetchData(days);

                    // Hentikan proses jika tidak ada data
                    if (!apiData || apiData.length === 0) {
                        if (!document.querySelector('#stats-chart p')) { // Cek jika sudah ada pesan error
                            document.getElementById('stats-chart').innerHTML =
                                '<p class="text-center p-10">Tidak ada data untuk ditampilkan.</p>';
                        }
                    ...
                    // Kosongkan container chart sebelum render baru
                    document.getElementById('stats-chart').innerHTML = '';

                    const categories = apiData.map(item => item.date);
                    const seriesData = apiData.map(item => item.count);
                    const totalBookings = seriesData.reduce((total, num) => total + num, 0);

                    // Update tampilan total booking
                    ...
                    const options = {
                        ...
                        yaxis: {
                            labels: {
                                formatter: (value) => Math.round(
                                    value) // Pastikan sumbu Y menampilkan angka bulat
                            }
                        },
                        dataLabels: {
                            enabled: false
                    ...
                    // Buat instance chart baru dan render
                    this.chart = new ApexCharts(document.getElementById("stats-chart"), options);
                    this.chart.render();
                },

                // Fungsi untuk meng-setup event listener
                init() {
                    // Muat chart awal
                    this.renderChart(7, 'Last 7 days');

                    // Tambahkan event listener ke setiap link di dropdown
                    document.querySelectorAll('#lastDaysdropdown a').forEach(link => {
                        link.addEventListener('click', (event) => {
                            ...
                            document.getElementById('dropdownDefaultButton').firstChild
                                .textContent = label + ' ';

                            // Render ulang chart dengan data baru
                            this.renderChart(days, label);
                        });
                    });
                }
            };
